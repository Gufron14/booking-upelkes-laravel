<div>
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="fw-bold">Kelola ruang</h2>
    ...
    <div class="card">
        ...
        <div class="card-body">
            <!-- Modal -->
            <div wire:ignore.self class="modal fade" id="ruangModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="ruangModalLabel">
                                @if($editingId) Edit Ruang @else Tambah Ruang @endif
                            </h5>
                        ...
                        <div class="modal-body">
                            <div class="d-flex gap-3 mb-4 align-items-center justify-content-center">
                                ...
                                <div class="col form-group">
                                    <label for="" class="form-label">Nomor ruang</label>
                                    <input type="text" class="form-control @error('kode_ruang') is-invalid @enderror"
                                        wire:model='kode_ruang'>
                                    @error('kode_ruang')
                                        <small class="text-danger">
                                            {{ $message }}
                                        </small>
                            ...
                            <div class="form-group mb-3">
                                <label for="images" class="form-label">Gambar Ruang</label>
                                ...
                                @if($editingId && isset($ruang))
                                    @php
                                        $currentRuang = collect($ruang)->firstWhere('id', $editingId);
                                    @endphp
                                    @if($currentRuang && $currentRuang->gambarRuangs && $currentRuang->gambarRuangs->count() > 0)
                                        <div class="mt-3">
                                            <label class="form-label">Gambar Saat Ini:</label>
                                            <div class="d-flex flex-wrap gap-2">
                                                @foreach($currentRuang->gambarRuangs as $gambar)
...
<script>
    document.addEventListener('livewire:initialized', () => {
        Livewire.on('showRuangModal', () => {
            const modal = new bootstrap.Modal(document.getElementById('ruangModal'));
            modal.show();
        });

        Livewire.on('hideRuangModal', () => {
            const modal = bootstrap.Modal.getInstance(document.getElementById('ruangModal'));
            if (modal) {
                modal.hide();
            }
        });
