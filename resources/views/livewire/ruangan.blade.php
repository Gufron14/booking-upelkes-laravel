    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h2 class="fw-bold text-primary mb-1">Daftar Ruangan</h2>
                    <p class="text-muted mb-0">Pilih ruangan sesuai kebutuhan Anda</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row mb-4">
        <div class="col-md-6">
            <div class="input-group">
                ...
                <input type="text" class="form-control border-start-0 ps-0" placeholder="Cari ruangan..."
            ...
            <div class="col-lg-4 col-md-6">
                <div class="card h-100 shadow-sm border-0 position-relative overflow-hidden">
                    ...
                    <div class="position-absolute top-0 start-0 z-3 m-3">
                        <span
                            class="badge {{ $ruang->layanan->kategori == 'pemerintah' ? 'bg-success' : 'bg-primary' }} px-3 py-2 rounded-pill">
                            <i class="fas {{ $ruang->layanan->kategori == 'pemerintah' ? 'fa-building' : 'fa-users' }} me-1"></i>
                            {{ ucfirst($ruang->layanan->kategori) }}
                        </span>
                    </div>
                        ...
                        @else
                            <div class="bg-gradient-primary d-flex align-items-center justify-content-center"
                                ...
                                <div class="text-center text-muted">
                                    ...
                                    <p class="mb-0 text-light">{{ $ruang->layanan->nama }}</p>
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="card-body p-4">
                        <!-- Nama ruang -->
                        <h5 class="card-title fw-bold text-dark mb-2">{{ $ruang->layanan->nama_layanan }}</h5>

                        <!-- Deskripsi -->
                        <p class="card-text text-muted mb-3" style="font-size: 0.9rem;">
                            {{ Str::limit($ruang->layanan->deskripsi, 100) }}
                        </p>
                            ...
                            <div class="col-6">
                                <div class="d-flex align-items-center">
                                    ...
                                    <small class="text-muted">
                                        <strong>{{ $ruang->layanan->kapasitas ?? 'N/A' }}</strong> orang
                                    </small>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="d-flex align-items-center">
                                    ...
                                    <small class="text-muted">
                                        <strong>1</strong> Ruangan
                                    </small>
                                </div>
                            </div>
                        </div>

                        @if ($ruang->layanan->fasilitas->count() > 0)
                            <div class="mb-3">
                                <small class="text-muted fw-semibold">Fasilitas:</small>
                                <div class="d-flex flex-wrap gap-1 mt-1">
                                    @foreach ($ruang->layanan->fasilitas->take(5) as $fasilitas)
                                        <span class="badge bg-light text-dark border">
                                            {{ $fasilitas->nama }}
                                        </span>
                                    @endforeach
                                    @if ($ruang->layanan->fasilitas->count() > 5)
                                        <span class="badge bg-secondary">
                                            +{{ $ruang->layanan->fasilitas->count() - 3 }}
                                        </span>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h4 class="text-primary fw-bold mb-0">
                                    Rp {{ number_format($ruang->layanan->tarif, 0, ',', '.') }}
                                </h4>
                                <small class="text-muted">
                            ...
                            <div class="btn-group">
                                <button class="btn btn-outline-primary btn-sm"
                                    wire:click="selectRuang({{ $ruang->id }})" data-bs-toggle="modal"
        ...
        @empty
            <div class="col-12">
                <div class="text-center py-5">
                    ...
                    <h4 class="text-muted">Tidak ada ruangan ditemukan</h4>
                ...
                <div class="modal-body">
                    @if($selectedRuangForCart)
                        ...
                        @if($selectedRuang)
                            <div class="row mb-4">
                                <div class="col-md-4">
                                    @if($selectedRuang->gambarRuangs->count() > 0)
                                        <img src="{{ asset('storage/' . $selectedRuang->gambarRuangs->first()->path) }}"
                                            class="img-fluid rounded" alt="{{ $selectedRuang->kode_ruang }}">
                                    @else
                                        <div class="bg-light d-flex align-items-center justify-content-center rounded"
                                            style="height: 150px;">
